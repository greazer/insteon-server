		...
		wss.on('connection', function (ws) {
			...
			devices.forEach(function (device) {
				...
				switch (device.deviceType) {
					case 'motionsensor':
						console.log('XXXXXXXXXXXXXXXXXXMotionSensorXXXXXXXXXXXXXXX')
						break;
						...
						if (isDevice) {
							...
							for (var i = 0, len = foundDevices.length; i < len; i++) {
								...
								switch (foundDevice.deviceType) {
									case 'motionsensor':
										if (command1 !== '06') 
											console.log('Motion sensor command1: ' + command1 + ' command2: ' + command2)
										...
										console.log('Motion Sensor message: ' + JSON.stringify(message))
										if (ws.isAlive) { ws.send(JSON.stringify(message)) }
										break

									case 'lightbulb':
									case 'dimmer':
										console.log('Got lightbulb/dimmer event for ' + foundDevice.name + ' (' + foundDevice.deviceID + ')')
										if (command1 == '19' || command1 == '03' || command1 == '04' || (command1 == '00' && command2 != '00') || (command1 == '06' && messageType == '1')) { //19 = status
											...
											message = { name: foundDevice.name, button: foundDevice.keypadbtn, id: foundDevice.deviceID, deviceType: foundDevice.deviceType, gatewayId: gatewayId, raw: raw, state: level }
											console.log('Light/Dimmer message: ' + JSON.stringify(message))
											if (ws.isAlive) { ws.send(JSON.stringify(message)) }
										}
